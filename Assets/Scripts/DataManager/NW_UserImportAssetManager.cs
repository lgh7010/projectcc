...
public class NW_UserImportAssetManager : MonoBehaviour {
    ...
    private class UserImportAssetInfo {
        public object asset;
    ...
    /// </summary>
    public Dictionary<string, List<Texture2D>> tempTextureDic = new Dictionary<string, List<Texture2D>>();// [usage / texture]
    /// <summary>
    /// 유저 에셋 폴더의 모든 하위폴더에 존재하는 모든 오디오 클립을 usage별로 나열합니다. 각 usage의 리스트에 2개 이상의 오디오 클립이 들어있는 경우, 해당 오디오 클립의 Usage가 충돌한 것입니다.
    /// </summary>
    public Dictionary<string, List<AudioClip>> tempAudioClipDic = new Dictionary<string, List<AudioClip>>();// [usage / audioSource]

    /// <summary>
    /// 유저 에셋 폴더에서 임포트된 모든 usage별 텍스쳐
    /// </summary>
    public static Dictionary<string, Texture2D> UserImportTextureDic = new Dictionary<string, Texture2D>();
    public static Dictionary<string, AudioClip> UserImportAudioClipDic = new Dictionary<string, AudioClip>();

    private void Awake() {
        this.Initialize_UserImportAssets();

        //인게임 오브젝트 테스트 코드
        GameObject.Find("Cube1").GetComponent<MeshRenderer>().material.mainTexture = UserImportTextureDic["usage1"];
        GameObject.Find("Cube2").GetComponent<MeshRenderer>().material.mainTexture = UserImportTextureDic["usage2"];
        GameObject.Find("Cube3").GetComponent<MeshRenderer>().material.mainTexture = UserImportTextureDic["usage1"];
        ...
        Rect rec = new Rect(0, 0, UserImportTextureDic["usage1"].width, UserImportTextureDic["usage1"].height);
        GameObject.Find("Image (1)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage1"], rec, new Vector2(0, 0), 1);
        GameObject.Find("Image (2)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage1"], rec, new Vector2(0, 0), 1);
        GameObject.Find("Image (3)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage1"], rec, new Vector2(0, 0), 1);
        GameObject.Find("Image (4)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage1"], rec, new Vector2(0, 0), 1);

        Rect rec2 = new Rect(0, 0, UserImportTextureDic["usage2"].width, UserImportTextureDic["usage2"].height);
        GameObject.Find("Image (5)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage2"], rec2, new Vector2(0, 0), 1);
        GameObject.Find("Image (6)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage2"], rec2, new Vector2(0, 0), 1);
        GameObject.Find("Image (7)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage2"], rec2, new Vector2(0, 0), 1);
        GameObject.Find("Image (8)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage2"], rec2, new Vector2(0, 0), 1);
        GameObject.Find("Image (9)").GetComponent<Image>().sprite = Sprite.Create(UserImportTextureDic["usage2"], rec2, new Vector2(0, 0), 1);
    }
    ...
    public void Initialize_UserImportAssets() {
        ...
        foreach (KeyValuePair<string, List<Texture2D>> kv in this.tempTextureDic){
            UserImportTextureDic[kv.Key] = kv.Value[0];
            Debug.Log("texture added in FinalTextureList. key : " + kv.Key + ", name : " + kv.Value[0].name);
        }
        foreach(KeyValuePair<string, List<AudioClip>> kv in this.tempAudioClipDic) {
        ...
        try {
            ...
            foreach (KeyValuePair<string, string> kv in tempDic) {
                if (!this.tempTextureDic.ContainsKey(kv.Key)) {
                    this.tempTextureDic.Add(kv.Key, new List<Texture2D>());
                }
                this.tempTextureDic[kv.Key].Add(WWWLoadUserImportAsset(kv.Key, Path.Combine(assetPath, kv.Value), UserImportAssetType.Texture2D) as Texture2D);
            }
        } catch {
        ...
        try {
            ...
            foreach (KeyValuePair<string, string> kv in tempDic) {
                if (!this.tempAudioClipDic.ContainsKey(kv.Key)) {
                    this.tempAudioClipDic.Add(kv.Key, new List<AudioClip>());
                }
                this.tempAudioClipDic[kv.Key].Add(WWWLoadUserImportAsset(kv.Key, Path.Combine(assetPath, kv.Value), UserImportAssetType.AudioClip) as AudioClip);
            }
        } catch {
